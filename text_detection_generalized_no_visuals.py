...
# Execution
target_image_path = 'images/testcase3.png' # testcase5.png good for vertical text, testcase3.png good for horizontal text
target_img, original_shape = load_image(target_image_path)
easyocr_img = target_img.copy()
forbidden_chars = ['<','.','>',',','/',';',':','\'','"','[',']','{','}','|','\\','`','~','!','^','(',')','_','-','+','=']

reader = create_ocr_reader(['en'])
result = extract_text_from_image(reader, easyocr_img)

# Rotate the image by 90 degrees counterclockwise and search for text
rotated_img = rotate_image(easyocr_img, 90)
rotated_result = extract_text_from_image(reader, rotated_img)
